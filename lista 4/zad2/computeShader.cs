...
layout(rgba32f, binding = 4) uniform writeonly image2D img;
layout(local_size_x = 20, local_size_y = 20, local_size_z = 1) in;

float dt = 0.123456789;
...
void main()
{
    ...
    // dt tutaj zmienia parametr h !!!!!!
    // f and k dependent on "x" position
    float h = 0.5 * i / float(W) * dt;
    f = 0.02 * h + (1 - h) * 0.018;
    k = 0.035 * h + (1 - h) * 0.051;
    ...
    // Gray Scott model
    A2[idx0] = A1[idx0] + (DA * laplA - A1[idx0] * B1[idx0] * B1[idx0] + f * (1 - A1[idx0]));
    B2[idx0] = B1[idx0] + (DB * laplB + A1[idx0] * B1[idx0] * B1[idx0] - (k + f) * B1[idx0]);
